...
class Reformulator(object):
  ...
  def __init__(self, hparams_path, source_prefix, out_dir,
    ...
    with combined_graph.as_default():
      ...
      _, global_step = model_helper.create_or_load_model( self.train_model.model, p1 ,self.sess,"train")
  ...
  def tokenize(self, questions, prefix=""): #""):   EDITED
    ...
    if self.hparams.subword_option == "spm":
      ...
      for question in questions:
        ...
        # print('question type:', str(self.sentpiece.EncodeAsPieces(question.encode("utf-8"))))
        tokenized_questions.append(b" ".join(
            self.sentpiece.EncodeAsPieces(question.encode("utf-8"))))
    else:
      tokenized_questions = questions
    if prefix:
      # print('tokenized_questions', tokenized_questions, 'prefix:',prefix)
      # Note that no extra space is added between the prefix and the question.
      tokenized_questions = [prefix.encode("utf-8") + q for q in tokenized_questions]
  ...
  def detokenize(self, text):
    ...
    """
    # print('text:', type(text.split()[0]), text.split() )
  ...
  def reformulate(self, questions, inference_mode, trie_excludes=None):
    ...
    tokenized_questions = self.tokenize(questions, prefix=self.source_prefix) ##self.source_prefix) EDITED
